...
async function Data (){
    ...
    grid.addEventListener("click",(event)=>{
      ...
      let current=portfolio_item;
      current.lastChild.classList.add("flex");
     // console.log(portfolio_item.lastChild);
      ...
      after.addEventListener("click",(g)=>{
        g.preventProbagation();
        //console.log(current.nextElementSibling);
        let next=current.nextElementSibling.lastChild;
        let modal_ =grid.querySelector(".flex");
        next.classList.add("flex");
        modal_.classList.remove("flex");
        console.log(next,modal_);
