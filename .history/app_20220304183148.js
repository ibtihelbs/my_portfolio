...
async function Data (){
    ...
    let inc=(e)=>e.includes("CSS3")==true;
    console.log(tech_.findIndex(inc), 'is tech');
    console.table(tech_);
    ...
    let value=e.target.value.toLowerCase();
     ...
     tech_.forEach((col,index)=>{
       let visible=col.forEach(str=>str.toLowerCase().includes(value));
       //let visible=col.contains(value);
       console.log(value ,col.toLowerCase(), visible);
       item_portfolio[index].classList.toggle("hidden",!visible);
      })
     //let item=user.map(t=>t.item);
